...
const QualityMenu = ({ 
  ...
  captions, activeCaptionId, onCaptionChange
}) => {
  const [anchorEl, setAnchorEl] = useState(null);
  const [view, setView] = useState("main"); // "main", "quality", "speed", "captions"
      ...
      >
        {view === "main" && (
          <Box sx={{ py: 1 }}>
            ...
            {captions && captions.length > 0 && (
              <MenuItem onClick={() => setView("captions")} sx={{ display: 'flex', alignItems: 'center', py: 1.5, '&:hover': { bgcolor: 'rgba(255,255,255,0.1)' } }}>
                <SubtitlesIcon fontSize="small" sx={{ mr: 2, color: 'text.secondary' }} />
                <Typography variant="body2" sx={{ flexGrow: 1 }}>Captions</Typography>
                <Typography variant="caption" sx={{ color: 'text.secondary', mr: 1 }}>
                  {activeCaptionId ? captions.find(c => c.id === activeCaptionId)?.label || "On" : "Off"}
                </Typography>
                <KeyboardArrowRightIcon fontSize="small" sx={{ color: "text.secondary" }} />
              </MenuItem>
            )}
        ...
        {view === "captions" && (
          <Box sx={{ pb: 1, pt: 1 }}>
            <MenuItem onClick={() => setView("main")} sx={{ borderBottom: "1px solid #444", mb: 1, pb: 1.5, '&:hover': { bgcolor: 'rgba(255,255,255,0.1)' } }}>
              <ArrowBackIcon fontSize="small" sx={{ mr: 2 }} />
              <Typography variant="body2" sx={{ fontWeight: 600 }}>Captions</Typography>
            </MenuItem>
            <Box sx={{ maxHeight: 300, overflow: 'auto' }}>
              <MenuItem 
                onClick={() => { onCaptionChange(null); handleClose(); }}
                sx={{ py: 1.2, '&:hover': { bgcolor: 'rgba(255,255,255,0.1)' } }}
              >
                <Box sx={{ width: 32, display: 'flex', alignItems: 'center' }}>
                  {!activeCaptionId && <CheckIcon fontSize="small" />}
                </Box>
              ...
              {captions?.map((c) => (
                <MenuItem 
                  key={c.id}
                  onClick={() => { onCaptionChange(c.id); handleClose(); }}
                  sx={{ py: 1.2, '&:hover': { bgcolor: 'rgba(255,255,255,0.1)' } }}
                >
                  <Box sx={{ width: 32, display: 'flex', alignItems: 'center' }}>
                    {activeCaptionId === c.id && <CheckIcon fontSize="small" />}
                  </Box>
