...
import useCreateMediaFile from "../../../../../../hooks/media-file-hooks/useCreateMediaFile";
import useAddLinkToMF from "../../../../../../hooks/media-file-hooks/useAddLinkToMF";
...
function ResourceContent({ item, onUpdate, onClose }) {
  ...
  const addLinkToMF = useAddLinkToMF();
  ...
  const handleUploadFile = async (file, override = false) => {
    ...
    if (fileSize > maxSize) {
      ...
      const result = await createMediaFile.mutateAsync({
        file,
        isOverride: override,
        courseId: courseId ? parseInt(courseId) : null
      });
  ...
  const handleAddLink = async () => {
    ...
    const exists = await checkFileExists.mutateAsync({ fileName: linkTitle });
  ...
  const handleConfirmDelete = async () => {
    if (pendingDeleteContent) {
      await deleteMediaFile.mutateAsync(pendingDeleteContent.id);
    }
