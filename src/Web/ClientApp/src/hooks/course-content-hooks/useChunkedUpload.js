  ...
  return useMutation({
    mutationFn: async ({ file, fileHash = "", courseId = null, onProgress = null }) => {
      try {
        ...
        const initiateResponse = await fetch("/api/CourseContent/chunks/initiate", {
          method: "POST",
          headers: {
        ...
        for (let chunkNumber = uploadedChunks; chunkNumber < totalChunks; chunkNumber++) {
          ...
          for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
            try {
              const chunkResponse = await fetch("/api/CourseContent/chunks/upload", {
                method: "POST",
                headers: {
                  Authorization: `Bearer ${token}`,
                },
        ...
        const statusResponse = await fetch(
          `/api/CourseContent/chunks/${uploadSession.sessionId}/status`,
          {
            method: "GET",
            headers: {
              Authorization: `Bearer ${token}`,
