...
public static class CourseSeedData
{
        ...
        string ins1, string ins2, string ins3)
    {
                 ...
                 int cat, string by, string obj, params int[] tIds)
        {
            var c = new Course
            {
                Title = title, Subtitle = sub, Level = lvl, Status = CourseStatus.Public,
                Ratings = rat, TotalStudents = stu, TotalRatingStudent = stu > 0 ? stu / 10 : 1,
                TotalRating = (int)(rat * (stu > 0 ? stu / 10 : 1)),
                CategoryId = cat, CreatedBy = by, LastModifiedBy = by,
                Price = 0f, LearningObjectives = obj, Requirements = "[]", TargetAudience = "[]",
