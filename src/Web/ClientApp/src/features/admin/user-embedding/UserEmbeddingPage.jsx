...
import useBatchEmbedUsers from "../../../hooks/user-embedding-hooks/useBatchEmbedUsers";
import useGetUserEmbeddingSyncStatus from "../../../hooks/user-embedding-hooks/useGetUserEmbeddingSyncStatus";
import useEmbedSingleUser from "../../../hooks/user-embedding-hooks/useEmbedSingleUser";
import useDebounce from "../../../hooks/common/useDebounce";

import UserEmbeddingStatsBar from "./components/UserEmbeddingStatsBar";
import UserEmbeddingRow from "./components/UserEmbeddingRow";

// ─── Constants ────────────────────────────────────────────────────────────────

const HEAD_LABEL = [
  { id: "userId", label: "User ID", width: 100 },
  { id: "user", label: "User", minWidth: 240 },
...
function UserEmbeddingPage() {
  ...
  const { data: syncData, isLoading: isSyncLoading, refetch } =
    useGetUserEmbeddingSyncStatus(debouncedSearch, filter, page + 1, rowsPerPage);

  const { mutate: batchEmbed, isPending: isBatching } = useBatchEmbedUsers();
  const { mutate: embedSingle } = useEmbedSingleUser();
  ...
  const tabCounts = {
    All: syncData?.result?.totalUsers ?? 0,
  ...
  return (
    <Box sx={{ px: { xs: 2, sm: 3, md: "40px", lg: "120px", xl: "240px" } }}>
      ...
      <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
        <PageTitle title="User Embeddings" />
      </Box>
      <CustomBreadcrumbs />
      ...
      <Box sx={{ mt: 4, mb: 2, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 1.5 }}>
        ...
        <Button
          ...
          onClick={() => batchEmbed()}
          disabled={isBatching}
          id="batch-embed-users-btn"
          sx={primaryBtnSx}
        >
          {isBatching ? "Enqueuing…" : "Re-index All Users"}
        </Button>
      </Box>

      {/* ── Stats bar ── */}
      <UserEmbeddingStatsBar syncData={syncData} isLoading={isSyncLoading} />

      {/* ── Alerts ── */}
      {!isSyncLoading && missingCount > 0 && (
        <Alert severity="warning" sx={{ mb: 2, borderRadius: "12px" }}>
          <strong>{missingCount} user{missingCount > 1 ? "s" : ""}</strong> have learner profiles but are not yet embedded.
          Click <strong>Re-index All Users</strong> to fix this automatically, or embed them individually below.
        </Alert>
      )}
      {!isSyncLoading && missingCount === 0 && totalCount > 0 && (
        <Alert severity="success" sx={{ mb: 2, borderRadius: "12px" }}>
          All users with learner profiles are embedded and up to date.
        </Alert>
      )}

      <Alert severity="info" sx={{ mb: 2, borderRadius: "12px" }}>
        User profiles are embedded into the <strong>edunary_users</strong> Qdrant collection for personalized AI features.
        Embeddings auto-update when a user saves their learner profile or changes their full name.
      </Alert>
      ...
      <Card sx={cardSx}>
        ...
        <EmbeddingStatusTabs
          activeTab={filter}
          onChange={(val) => { if (val !== null) { setFilter(val); setPage(0); } }}
          counts={tabCounts}
        />
...
export default UserEmbeddingPage;
